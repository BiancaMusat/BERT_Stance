...
def get_data(train_df, val_df, test_df, weak_df):
	...
	x1_weak, x2_weak, y_weak = weak_df['premise'], weak_df['text'], weak_df['stance']
	...
	x1_weak, x2_weak, y_weak = x1_weak.tolist(), x2_weak.tolist(), y_weak.tolist()
	return (x1_train, x2_train, y_train), (x1_val, x2_val, y_val), (x1_test, x2_test, y_test), (x1_weak, x2_weak, y_weak)
...
def predict(prediction_dataloader, model):
    	# Prediction on test set
	...
	for batch in prediction_dataloader:
	  ...
	  batch_ids = re.findall('\(.*?\,',batch_indexes[1:-2])  # read indexes
...
def main():
	...
	(x1_train, x2_train, y_train), (x1_val, x2_val, y_val), (x1_test, x2_test, y_test), (x1_weak, x2_weak, y_weak) = get_data(train_df, val_df, test_df, weak_df)
	...
	weak_inputs, weak_masks, weak_token_ids = tokenize(model_name, x1_weak, x2_weak, tokenizer, MAX_LEN)
	...
	# Predict on Weak set
	predictions, indexes = predict(weak_dataloader, model)
